...
; - removed mouse record and time delay from defaults. 

; - stopped the gui from showing its head all the time.
;       and use F9 to show it

; - recording a new macro, doesn't pile on the old one. 

; #NoTrayIcon
; de cpu consuming bug!!!
; just bucause accidentally start recording...

#include E:\test\ahk\bright.ahk
...
Gui, Add, Checkbox, x52 y190 w75 h20 vWPos, Positions
Gui, Add, Checkbox, x132 y190 w50 h20 vWSize, Sizes
Gui, Add, Checkbox, x32 y230 w130 h20  vMouseMovemnets, Mouse Movements
Gui, Add, Checkbox, x32 y210 w100 h20 +Checked vClicks, Mouse Clicks
; Useful in Minecraft
; Gui, Add, CheckBox, x132 y210 w50 h20 vMousePos, Pos
Gui, Add, Checkbox, x32 y250 w130 h20 +Checked vKeyboardActions, Keyboard Actions

Gui, Add, GroupBox, x12 y145 w180 h135 , Record Options
Gui, Add, Text, x22 y20 w70 h20 , CoordMode
Gui, Add, Text, x22 y40 w70 h20 , BlockInput
Gui, Add, DropDownList, x92 y20 w90 h21 +R7 vCoordType, Screen||Window
Gui, Add, DropDownList, x92 y40 w90 h21 +R7 vBlockInputType, On|Off|MouseMove||

Gui, Add, GroupBox, x12 y70 w180 h70 , Hotkeys (F8 Repeat Play)
...
; TODO: add ctrl alt shift and win combination !!!
...
; Gui, Add, CheckBox, x32 y312 w140 h35 vAskRepeatTime, Ask Before Play (Alt+F7 Always Ask)
Gui, Add, Checkbox, x32 y270 w112 h20 +Checked vFixedDelay gFixedDelay, Fixed Delay
Gui, Add, Edit, x148 y270 w40 h20 +number vDefaultDelay, 50
Gui, Add, UpDown, x158 y270 w18 h20 , 50
; Gui, Add, Text, x32 y310 w170 h60 , Todo: changable for every play, add speed up when not fixed, and force fixed!

Gui, Add, GroupBox, x12 y295 w180 h85 , Play Options

; TODO: speed up
Gui, Add, Checkbox, x32 y310 w112 h20 vSpeedUp gSpeedUp, Speed Up
Gui, Add, Edit, x148 y310 w40 h20 +number +disabled vSpeedVar, 1
Gui, Add, UpDown, x158 y310 w18 h20 , 1

; TODO: force fixed
Gui, Add, Checkbox, x32 y330 w112 h20 vForceFixed gForceFixed, Force Fixed
Gui, Add, Edit, x148 y330 w40 h20 +number +disabled vForceVar, 50
Gui, Add, UpDown, x158 y330 w18 h20 , 50
...
TempVar .= XMR2AHK(XMR_Data,CoordType,BlockInputType) "`nreturn"
IfExist,% IniFile
	FileDelete,% IniFile
...
XMF(TempVar,WinInfo,WinInfo,WPos,WSize,MouseMovemnets,Clicks,KeyboardActions,FixedDelay,DefaultDelay)
XMR_Data := Trim(TempVar,"`n")
...
RepeatTime:=1
if AskRepeatTime
    InputBox, RepeatTime,Enter Times For Repeat,,,,,,,,,1
if ErrorLevel=1
    RepeatTime:=0
while (!StopXMPFlag && RepeatTime>=1)
{
...
FixedDelay:
GuiControlGet,FixedDelay,,FixedDelay
GuiControl,Enable%FixedDelay%,DefaultDelay
return


SpeedUp:
GuiControlGet,SpeedUp,,SpeedUp
GuiControl,Enable%SpeedUp%,SpeedVar
; msgbox,TODO1
return


ForceFixed:
GuiControlGet,ForceFixed,,ForceFixed
GuiControl,Enable%ForceFixed%,ForceVar
; msgbox,TODO2
return
...
Hotkey, F8 , PlayRecordRepeat, On UseErrorLevel
return
...
LControl|RControl|LShift|RShift|LAlt|RAlt|LWin|RWin"
	...
	Critical
		CoordMode,Mouse
		MouseGetPos,XPos,YPos ; add option to remove pos
...
XMF(ByRef XMR,WT=0,WC=0,WP=0,WS=0,M=0,C=0,K=0,F=0,DefaultDelay=0)
{
	Loop,Parse,XMR,`n,`r
		{
		W := WT||WC||WP||WS ? 1 : 0
	...
	Loop,Parse,XMR_Data,`n
	{
		...
		if (Info1 = "M") {
			;~ if (A_TickCount - FirstTickCount <= TotalTime)
				...
				MouseClick,% Detail1,% Detail2 + XPos,% Detail3 + YPos,1,0,% Detail4 ; change here to remove pos info!!
		else if (Info1 = "D")
		{
			TotalTime += Detail1
...
XMR2AHK(XMR,CoordType="Screen",BlockInputType="Off")
{
	...
	if (BlockInputType<>"Off")
		AHK_Data .= "`nBlockInput,Off`nBlockInput,MouseMoveOff"
...
#IfWinActive, XMR v2.1 ahk_exe AutoHotKey.exe
Esc::
; hideGUI:=1
; Gui,Hide
    WinMinimize, A
